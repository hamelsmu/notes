...
describe("parser", () => {
    it("parses blog pages", async () => {
        const htmlPath = path.join(__dirname, "..", "..", "..", "example-docs/build/blog/d-s-l-test/index.html");
        ...
        expect(html2text(html, "blog")).toEqual({
            ...
            sections: [
                ...
                },
                {
                    ...
                    content: 'FIRST SUBHEADER CONTENT #include <stdio.h> int main(int argc, char** argv) { printf("Hello World"); }',
    ...
    describe("docs", () => {
        it("parses normal pages", async () => {
            const htmlPath = path.join(__dirname, "..", "..", "..", "example-docs/build/docs/d-s-l-test/index.html");
            const html = await readFileAsync(htmlPath, "utf-8");
        ...
        it("parses empty page", async () => {
            const htmlPath = path.join(__dirname, "..", "..", "..", "example-docs/build/docs/d-s-l-test2/index.html");
            const html = await readFileAsync(htmlPath, "utf-8");
            expect(html2text(html, "docs")).toEqual({
                docSidebarParentCategories: ["Docusaurus"],
                pageTitle: "DOC TITLE",
        ...
        it("parses nested sidebar categories", async () => {
            const htmlPath = path.join(__dirname, "..", "..", "..", "example-docs/build/docs/next/nested_sidebar_doc/index.html");
            const html = await readFileAsync(htmlPath, "utf-8");
            expect(html2text(html, "docs")).toEqual({
        ...
        it("parses page with first header directly after title", async () => {
            const htmlPath = path.join(__dirname, "..", "..", "..", "example-docs/build/docs/d-s-l-test3/index.html");
            const html = await readFileAsync(htmlPath, "utf-8");
            expect(html2text(html, "docs")).toEqual({
                docSidebarParentCategories: ["Docusaurus"],
                pageTitle: "DOC TITLE",
